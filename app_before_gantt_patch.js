  ...
  tasks.forEach(t=>{
    ...
    const vendorBadges = (()=> {
      ...
      return Array.from(set).sort((a,b)=>a.localeCompare(b,"fr",{sensitivity:"base"}))
        .map(v=>`<span class="badge owner" style="background:#4b5563;border-color:#4b5563;color:#fff;">${v}</span>`).join(" ");
    })();

    html+=`<tr data-task="${t.id}">`;
    html+=`<td><b><span class="num-badge" style="--badge-color:${color};--badge-text:#fff;">${taskOrderMap[t.id]||""}</span> <span class="icon-picto">📌</span> ${taskTitle(t)}</b><div class="gantt-meta">${ownerBadges}</div></td>`;
...
function renderMasterGantt(){
  ...
  lanes.forEach(lane=>{
    ...
    const vendorSet = new Set(lane.tasks.map(t=>t.vendor||"").filter(Boolean));
    const hasInternal = lane.tasks.some(t=> (t.owner||"").toLowerCase().includes("interne"));
    if(hasInternal) vendorSet.add("Équipe interne");
    let vendors = Array.from(vendorSet).sort((a,b)=>a.localeCompare(b,"fr",{sensitivity:"base"}));
